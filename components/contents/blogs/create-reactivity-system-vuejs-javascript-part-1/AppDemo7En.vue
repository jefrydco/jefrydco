<template>
  <app-demo :path="DEFAULT_PATH" :name="$options.name">
    <div class="app-demo__card app-demo__card-7">
...
export default ExtendableCreateReactivity1.extend({
  ...
  methods: {
    init: onetime(function (script7: Element) {
      const newScript = document.createElement('script')
      const inlineScript = document.createTextNode(`if (!window.state7) {
          ...
          window.state7 = (function () {
            ...
            function main() {
              ...
              const stateDisplay = document.querySelector(
                '.app-demo__card-7 .state'
              )

              const resultDisplay = document.querySelector(
                '.app-demo__card-7 .result'
              )
